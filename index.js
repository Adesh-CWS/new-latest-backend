...
const welcomeEmployeeEmailTemplate =require("./template/welcomeEmployeeEmailTemplate")
...
const allowedOrigins = [
  ...
  "https://ems-cws-frontend.vercel.app",
];
...
dayStatus: record.dayStatus || "Present",
...
console.log("Calculated probationEndDate:", probationEndDate);
      ...
      const verifyLink = `https://ems-cws-frontend.vercel.app/employee/verify/${
        newEmployee._id
  ...
  try {
    ...
    const welcomeHtml = welcomeEmployeeEmailTemplate(newEmployee);

    await transporter.sendMail({
      from: `"CWS EMS" <${process.env.EMAIL_USER}>`,
      to: newEmployee.email,
      subject: "Welcome to Creative Web Solution",
      html: welcomeHtml,
    });

    console.log("Welcome mail sent after 10 min");
...
app.post("/admin/probation/extend/:employeeId", authenticate,async (req, res) => {
  try {
    ...
    const oldProbationDate = new Date(emp.probationEndDate);
    const parsedDate = new Date(newEndDate);

    const oldDate = new Date(oldProbationDate);
    ...
    let days = newDate.getDate() - oldDate.getDate();
    ...
    let extensionText = "Probation extended by ";

    if (months > 0) {
      extensionText += `${months} ${months === 1 ? "month" : "months"}`;
    }
    ...
    if (days > 0) {
      ...
      extensionText += `${days} ${days === 1 ? "day" : "days"}`;
    }
  ...
  //send mail 
  try {
    const emailHtml = await probationExtendedTemplate(emp.name, parsedDate, reason, extensionText);
    await transporter.sendMail({
      from: `"CWS EMS" <${process.env.EMAIL_USER}>`,
      to: emp.email,
...
// Restore Employee
app.put("/restoreEmployee/:id", authenticate, async (req, res) => {
  try {
...
app.post("/sendpasswordlink", async (req, res) => {
  ...
  try {
    ...
    const forLink = `https://ems-cws-frontend.vercel.app/forgotpassword/${userfind._id}/${setusertoken.verifytoken}`;
...
const getThisMonthLateCount = async (
  employeeId,
  currentDate
) => {
...
// const getThisMonthLateCount = async (employeeId, currentDate) => {
//   const monthStart = new Date(
//     currentDate.getFullYear(), 
//     currentDate.getMonth(), 1);
//   monthStart.setHours(0, 0, 0, 0);
...
app.post("/attendance/:id/checkin", authenticate, async (req, res) => {
  try {
    ...
    // if (!photo){
    //   return res.status(400).json({
    //     message : "Photo capture is required for check-in. Please capture your photo"
    //   })
    // }
    const today = getToday();
    ...
    // attendance.checkInPhoto = uploadedPhoto;
...
if (isLate) {
  ...
  const monthlyLateCount = await getThisMonthLateCount(id, today);

  const newLateCount = monthlyLateCount + 1;

  attendance.lateCheckInCount = newLateCount;


  // First 3 late allowed
  if (newLateCount <= 3) {

    attendance.dayStatus = "Late Check-In";

  } 
  // 4th onwards all late = Half Day
  else {

    attendance.dayStatus = "Late Check-In Half Day Penalty";
  ...
  attendance.dayStatus = "Present";
    ...
    attendance.dayStatus = attendance.dayStatus || "Present";

    await attendance.save();
    await autoSandwich(id);//rutuja code for Before and after weekend absent
...
app.post("/attendance/:id/checkout", authenticate, async (req, res) => {
  try {
    ...
    await attendance.save();
    await autoSandwich(req.params.id); //rutuja code for Before and after weekend absent
    res.json({ message: "Check-out successful", attendance });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: err.message });
...
async function calculateWithSandwich(
  ...
  //rutuja code start 09-06-26
  if (prev) {
    let beforeDate = new Date(start);
    beforeDate.setDate(beforeDate.getDate() - 1);

    while (isOffDay(beforeDate, holidays, weeklyOffData)) {
      extraDays++;
      beforeDate.setDate(beforeDate.getDate() - 1);
    }
  }

  // 🔥 Off days AFTER chain (UNCHANGED)
  if (next) {
    let afterDate = new Date(end);
    afterDate.setDate(afterDate.getDate() + 1);

    while (isOffDay(afterDate, holidays, weeklyOffData)) {
      extraDays++;
      afterDate.setDate(afterDate.getDate() + 1);
    }
  }
  //rutuja code  end 09-06-26
...
//rutuja code for Before and after weekend absent

const applySandwichPolicy = async (employeeId, targetDate = null) => {
  try {
    let friday = new Date();
    ...
    const fridayAtt = await Attendance.findOne({ employee: employeeId, date: friday });
    const mondayAtt = await Attendance.findOne({ employee: employeeId, date: monday });
    
    const fridayPresent = fridayAtt?.checkIn || fridayAtt?.regularizationRequest?.status === "Approved";
    const mondayPresent = mondayAtt?.checkIn || mondayAtt?.regularizationRequest?.status === "Approved";
    
    const existingLeave = await Leave.findOne({
      employee: employeeId,
      isSandwichApplied: true,
      leaveType: "LWP",
      dateFrom: { $lte: monday },
      dateTo: { $gte: friday }
    });
    
    if (existingLeave && (fridayPresent || mondayPresent)) {
      const employee = await User.findById(employeeId);
      if (employee) {
        let d = new Date(existingLeave.dateFrom);
        while (d <= existingLeave.dateTo) {
          await Attendance.updateOne(
            { employee: employeeId, date: d },
            { $unset: { leaveRef: "", leaveType: "", isSandwich: "" } }
          );
          d.setDate(d.getDate() + 1);
        }
        employee.LwpLeave = Math.max(0, employee.LwpLeave - existingLeave.totalDays);
        await employee.save();
        await Leave.findByIdAndDelete(existingLeave._id);
      }
      return;
    }
    
    // If both absent, create sandwich leave
    if (!fridayPresent && !mondayPresent && !existingLeave) {
      
      const saturday = new Date(friday);
      saturday.setDate(saturday.getDate() + 1);
      
      if (!isNthSaturdayOff(saturday, weeklyOffData)) {
        console.log("Saturday is working - No sandwich leave created");
        return;
      }
      ...
      if (lastLeave?.leaveId) {
        nextNum = parseInt(lastLeave.leaveId.replace("LV", "")) + 1;
      }
      
      const newLeave = new Leave({
        leaveId: `LV${String(nextNum).padStart(4, "0")}`,
        employee: employeeId,
        leaveType: "LWP",
        dateFrom: friday,
        dateTo: monday,
        duration: "full",
        reason: "Auto leave generated: Friday-Monday absent",
        status: "approved",
        approvedBy: employee.reportingManager || employeeId,
      ...
      let d = new Date(friday);
      while (d <= monday) {
        await Attendance.updateOne(
          { employee: employeeId, date: d },
          { $set: { dayStatus: "Leave", leaveType: "LWP", leaveRef: newLeave._id, isSandwich: true } },
          { upsert: true }
        );
        d.setDate(d.getDate() + 1);
      }
      ...
      // Update LWP balance
      employee.LwpLeave = (employee.LwpLeave || 0) + 4;
      await employee.save();
    }
  } catch (err) {
    console.error("Sandwich policy error:", err);
  }
};

// rutuja code for friday leave and monday absent
const applyFridayLeaveMondayAbsentSandwich = async (employeeId) => {
  try {
    let friday = new Date();
    while (friday.getDay() !== 5) {
      friday.setDate(friday.getDate() - 1);
    ...
    const hasApprovedLeaveOnFriday = !!fridayLeave;
    
    const fridayAtt = await Attendance.findOne({ employee: employeeId, date: friday });
    const fridayRegularizationApproved = fridayAtt?.regularizationRequest?.status === "Approved";
    
    const mondayAtt = await Attendance.findOne({ employee: employeeId, date: monday });
    const mondayCheckIn = mondayAtt?.checkIn;
    const mondayRegularizationApproved = mondayAtt?.regularizationRequest?.status === "Approved";
    const mondayLeave = await Leave.findOne({
      employee: employeeId,
      status: "approved",
      dateFrom: { $lte: monday },
      dateTo: { $gte: monday }
    ...
    const mondayAbsent = !mondayCheckIn && !mondayRegularizationApproved && !mondayLeave;
    
    const saturday = new Date(friday);
    saturday.setDate(saturday.getDate() + 1);
    const isSaturdayOff = isNthSaturdayOff(saturday, weeklyOffData);
    
    const existingAutoLeave = await Leave.findOne({
      employee: employeeId,
      isFridayLeaveMondayAbsentAutoApplied: true,
      leaveType: "LWP"
    });
    
    if (mondayRegularizationApproved && existingAutoLeave) {
      const employee = await User.findById(employeeId);
      if (employee) {
        let d = new Date(existingAutoLeave.dateFrom);
        while (d <= existingAutoLeave.dateTo) {
          await Attendance.updateOne(
            { employee: employeeId, date: d },
            { $unset: { leaveRef: "", leaveType: "", isSandwich: "" } }
          );
        ...
        employee.LwpLeave = Math.max(0, employee.LwpLeave - existingAutoLeave.totalDays);
        await employee.save();
        await Leave.findByIdAndDelete(existingAutoLeave._id);
        console.log(`Removed auto leave for ${employee.name}`);
      }
      return;
    }
    
    if (hasApprovedLeaveOnFriday && !fridayRegularizationApproved && mondayAbsent && isSaturdayOff && !existingAutoLeave) {
      const employee = await User.findById(employeeId);
      if (!employee) return;
      ...
      const newLeave = new Leave({
        ...
        dateTo: monday,
        duration: "full",
        reason: `Auto leave generated: Approved leave on Friday, Absent on Monday`,
        status: "approved",
        approvedBy: employee.reportingManager || employeeId,
        approvedByRole: "system",
        totalDays: 3,
      ...
      let current = new Date(saturday);
      while (current <= monday) {
        const existingAtt = await Attendance.findOne({ employee: employeeId, date: current });
        if (!existingAtt?.checkIn && !existingAtt?.leaveRef) {
          await Attendance.updateOne(
            { employee: employeeId, date: current },
            { $set: { dayStatus: "Leave", leaveType: "LWP", leaveRef: newLeave._id, isSandwich: true } },
            { upsert: true }
          );
        }
      ...
      employee.LwpLeave = (employee.LwpLeave || 0) + 3;
      await employee.save();
      console.log(`Auto leave created (Fri Leave + Mon Absent) for ${employee.name}`);
    }
    
  } catch (err) {
    console.error("Friday Leave + Monday Absent error:", err);
  }
};


// rutuja code for friday absent and monday leave 
const applyFridayAbsentMondayLeaveSandwich = async (employeeId) => {
  try {
    let friday = new Date();
    while (friday.getDay() !== 5) {
      friday.setDate(friday.getDate() - 1);
    ...
    const fridayAtt = await Attendance.findOne({ employee: employeeId, date: friday });
    const fridayCheckIn = fridayAtt?.checkIn;
    const fridayRegularizationApproved = fridayAtt?.regularizationRequest?.status === "Approved";
    const fridayLeave = await Leave.findOne({
      employee: employeeId,
      status: "approved",
    ...
    const fridayAbsent = !fridayCheckIn && !fridayRegularizationApproved && !fridayLeave;
    ...
    const hasApprovedLeaveOnMonday = !!mondayLeave;
    ...
    const existingAutoLeave = await Leave.findOne({
      employee: employeeId,
      isFridayAbsentMondayLeaveAutoApplied: true,
      leaveType: "LWP"
    });
    
    if (fridayRegularizationApproved && existingAutoLeave) {
      const employee = await User.findById(employeeId);
      if (employee) {
        let d = new Date(existingAutoLeave.dateFrom);
        while (d <= existingAutoLeave.dateTo) {
    ...
    if (fridayAbsent && hasApprovedLeaveOnMonday && !mondayRegularizationApproved && isSaturdayOff && !existingAutoLeave) {
      const employee = await User.findById(employeeId);
      if (!employee) return;
      ...
      const newLeave = new Leave({
        ...
        dateTo: sunday,
        duration: "full",
        reason: `Auto leave generated: Absent on Friday, Approved leave on Monday`,
        status: "approved",
        approvedBy: employee.reportingManager || employeeId,
        approvedByRole: "system",
        totalDays: 3,
      ...
      employee.LwpLeave = (employee.LwpLeave || 0) + 3;
      await employee.save();
      console.log(`Auto leave created (Fri Absent + Mon Leave) for ${employee.name} `);
    }
    
  } catch (err) {
    console.error("Friday Absent + Monday Leave error:", err);
  }
};


// Auto trigger function
const autoSandwich = async (employeeId) => {
  setTimeout(async () => {
    await applySandwichPolicy(employeeId);

    await applyFridayLeaveMondayAbsentSandwich(employeeId);

    await applyFridayAbsentMondayLeaveSandwich(employeeId);

  }, 500);
};

//rutuja code end for Before and after weekend absent
...
async function saveChain(
  ...
  if (chain.length > 1) {
    ...
    while (isOffDay(before, holidays, weeklyOffData)) {
      start = new Date(before);
      before.setDate(before.getDate() - 1);
    }
    ...
    while (isOffDay(temp, holidays, weeklyOffData)) {
      end = new Date(temp);
      temp.setDate(temp.getDate() + 1);
    }
  }
...
// ========== END EMAIL SENDING ==========
    ...
    //rutuja code for sandwich senario
    if (status === "approved" || status === "rejected") {
      await autoSandwich(employee._id);
    }
    // ============================
  ...
  actionReason?.trim();
    ...
    record.regularizationRequest.approvedByName = approver.name;
      ...
      if (status === "Approved") {
        ...
        const regularizedHour = record.checkIn.getHours();
        const regularizedMinute = record.checkIn.getMinutes();
        const isRegularizedLate = regularizedHour > 9 || (regularizedHour === 9 && regularizedMinute > 10);
        
        if (!isRegularizedLate) {
          record.lateCheckIn = false;
          record.lateCheckInCount = 0;
          
          const monthStart = getMonthStart(record.date);
          ...
          for (let i = 0; i < lateRecords.length; i++) {
            const newCount = i + 1;
            lateRecords[i].lateCheckInCount = newCount;
            
            if (newCount % 3 === 0) {
              lateRecords[i].dayStatus = "Half Day";
            } else {
              lateRecords[i].dayStatus = "Present";
            }
            
            await lateRecords[i].save();
          }
          ...
          const newLateCount = monthlyLateCount + 1;
          record.lateCheckInCount = newLateCount;
          
          if (newLateCount % 3 === 0) {
            record.dayStatus = "Half Day";
          } else {
            record.dayStatus = record.workingHours >= 7.5 ? "Present" : "Half Day";
          }
        }
        await autoSandwich(record.employee._id); //rutuja code for Before and after weekend absent
      }
  ...
  "_id name email department designation role employeeId reportingManager",
    ...
    const employeesWithTodayAttendance = employees.map((emp) => {
      ...
      return {
        ...
        // checkInPhoto: rec ? rec.checkInPhoto : null,
...
app.put("/taskstatus/update/:id", async (req, res) => {
  try {
    ...
    await status.save();
// ✅ Update task progress when status becomes Completed
    if (cleanName.toLowerCase() === "completed") {
...
app.put("/task/:taskId/status", async (req, res) => {
  try {
    ...
    const isCompleted =
      statusDoc?.name?.toLowerCase() === "completed";
    ...
    if (task.assignedTo?.reportingManager) {
      managerId = task.assignedTo.reportingManager;
    }
    ...
    if (managerId) {
      await TaskNotification.create({
        ...
        type: "Task_Status_Update",
        message:
          `${task.assignedTo.name} updated task "${task.taskName}" status to "${newStatus.name}"`,
        taskRef: task._id,
        isRead: false,
      });
    }
    ...
    if (task.createdBy?._id) {
      ...
      if (
        ...
        await TaskNotification.create({
          ...
          type: "Task_Status_Update",
          message:
            `${task.assignedTo?.name || "Employee"} updated task "${task.taskName}" status to "${newStatus.name}"`,
          taskRef: task._id,
          isRead: false,
        });
...
// ✅ Create Policy (POST)
app.post("/policy/create", upload.fields([
  { name: "pdf", maxCount: 1 },
  ...
  try {
    ...
    const policy = new Policy({
      title,
      description,
      image: req.files?.pdf?.[0]?.path || null,
      videoUrl: req.files?.video?.[0]?.path || null,
    });
...
app.put(
  ...
  async (req, res) => {
    try {
      ...
      if (req.files?.pdf?.[0]) {
        updateData.image = req.files.pdf[0].path;
      }
...
const Icon = "https://res.cloudinary.com/dtkuzqegx/image/upload/v1781767056/Rectangle_158021_p7qypw.png";
   const dateIcon  = "https://res.cloudinary.com/dtkuzqegx/image/upload/v1781768521/Rectangle_158022_qneuyt.png"
   const timeIcon="https://res.cloudinary.com/dtkuzqegx/image/upload/v1781769113/Rectangle_158023_mjuuh5.png"
   const modeIcon= "https://res.cloudinary.com/dtkuzqegx/image/upload/v1781769185/Rectangle_158024_kd6e8y.png"
   const meetingIcon = "https://res.cloudinary.com/dtkuzqegx/image/upload/v1781769272/Rectangle_158025_kcoxsr.png"
   const positionIcon = "https://res.cloudinary.com/dtkuzqegx/image/upload/v1781769338/Rectangle_158026_athcbc.png"
   const interveiwIcon = "https://res.cloudinary.com/dtkuzqegx/image/upload/v1781769430/Rectangle_158027_zrkers.png"
   const noteIcon =" https://res.cloudinary.com/dtkuzqegx/image/upload/v1781769517/Rectangle_158028_ee92ph.png"
   const bellIcon = "https://res.cloudinary.com/dtkuzqegx/image/upload/v1781769495/Rectangle_158028_1_lt0qps.png"
...
font-size:11px;
">
Important information about your upcoming interview
</div>
...
/>
    ...
    <tr>
      <td style="padding:35px 40px;">
        ...
        <p style="font-size:16px;color:#333;margin-top:0;">
          Dear <strong>${interview.candidateName}</strong>,
        </p>

        <p style="font-size:15px;line-height:24px;color:#555;">
          We are pleased to inform you that an interview has been scheduled for
          the position you applied for. Please find the interview details below.
        </p>
  ...
  <td width="3%"
      ...
      font-size:14px;
      ">
              ${formatTime12Hour(interview.startTime)}
              -
              ${formatTime12Hour(interview.endTime)}
            </td>
          </tr>
...
">
  ${
    interview.interviewType === "Online"
      ? `Online (${getPlatformName(interview.link)})`
...
Kindly ensure you join the meeting on time.
In case of any queries, feel free to reach out to the HR team.
...
This email is sent to inform all relevant stakeholders
about the scheduled interview, ensuring everyone is
...
<hr></hr>
        <p style="margin-top:30px;color:#555;line-height:24px;">
...
`,
      ...
      // ================= INTERVIEWER MAIL =================
const intIcon ="https://res.cloudinary.com/dtxmxdwuf/image/upload/v1781851046/Rectangle_158021_nzla5q.png"
      const candidateIcon="https://res.cloudinary.com/dtxmxdwuf/image/upload/v1781844267/Rectangle_158022_g9yixm.png"
const roleIcon="https://res.cloudinary.com/dtxmxdwuf/image/upload/v1781844303/Rectangle_158022_1_wzt3yt.png"
const durationIcon="https://res.cloudinary.com/dtxmxdwuf/image/upload/v1781844352/Rectangle_158026_c2z57p.png"
const interviewerIcon="https://res.cloudinary.com/dtxmxdwuf/image/upload/v1781844387/Rectangle_158027_1_tvk9yz.png"
     const interviewerMailOptions = {
from: process.env.EMAIL_USER,
to: interviewer.email,
subject: `Interviewer Assignment Notification`,
...
/>
    ...
    <tr>
      <td style="padding:35px 40px;">
        ...
        <p style="font-size:16px;color:#333;margin-top:0;">
          Dear <strong>${interview.interviewerName}</strong>,
        </p>
        <p>
          This is to inform you that the interview round for a
        <strong>${interview.role}</strong> position has been scheduled as per the details below:
        </p>
           <!-- Interview Details -->
        <table width="100%" cellpadding="12" cellspacing="0"
          style="margin-top:25px;">
...
${interview.link}
          ...
          <hr></hr>
          <p style="margin-top:30px;color:#555;line-height:24px;">
     Please find the attached resume of the candidate for your review and reference.
        </p>
<p style="margin-top:30px;color:#555;line-height:24px;">
...
attachments,
      ...
      // ================= HR MAIL =================
const hrLogo="https://res.cloudinary.com/dtxmxdwuf/image/upload/v1781856513/Rectangle_158021_1_rf5nap.png"
const iconInterviewer="https://res.cloudinary.com/dtxmxdwuf/image/upload/v1781856446/Rectangle_158027_s2rnx3.png"
    const hrMailOptions = {
from: process.env.EMAIL_USER,
 ...
 <!-- Body -->
    <tr>
      <td style="padding:35px 40px;">
        ...
        <p>
         This is to confirm that the interview has been successfully scheduled
        </p>
           <!-- Interview Details -->
        <table width="100%" cellpadding="12" cellspacing="0"
          style="margin-top:25px;">
...
<hr></hr>
    <p style="margin-top:30px;color:#555;line-height:24px;">
    The candidate resume and interview details have been shared successfully with the 
respective interviewer and candidate
        </p>
...
<head>
<meta charset="UTF-8">
<title>Interview Scheduled Reminder</title>
</head>
...
">
  This is a friendly reminder regarding your upcoming interview for the position of
    <strong>${interview.role}</strong> at Creative Web Solution.
</div>
...
<head>
<meta charset="UTF-8">
<title>Interview Assignment Reminder</title>
</head>
...
font-size:11px;
">
Interview Assignment Notification Reminder
</div>
...
<!-- Body -->
    ...
    <strong>${interview.candidateName}</strong> for the position of
...
font-size:11px;
">
HR Confirmation Reminder Notification
</div>
